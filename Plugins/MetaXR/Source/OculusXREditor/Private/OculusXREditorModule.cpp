...
EVisibility FOculusXREditorModule::GetXRSimSectionVisiblity() const
{
	if (GetMutableDefault<UOculusXRHMDRuntimeSettings>()->bOverrideXRSimulatorVersion)
	{
		return EVisibility::Visible;
	}

	return FOculusXRHMDModule::IsXRSim2Installed() ? EVisibility::Hidden : EVisibility::Visible;
}
...
TSharedRef<SWidget> FOculusXREditorModule::CreateXrSimToolbarEntryMenu(TSharedPtr<class FUICommandList> Commands)
{
	...
	MenuBuilder.BeginSection(
		"SES",
		LOCTEXT("SES", "SES"),
		TAttribute<EVisibility>::Create(TAttribute<EVisibility>::FGetter::CreateLambda([this]() -> EVisibility {
			return GetXRSimSectionVisiblity();
		})));
	...
	[
		...
		[
			...
			[
				...
				.Visibility_Lambda([]()
				{
					return GetMutableDefault<UOculusXRHMDRuntimeSettings>()->bOverrideXRSimulatorVersion ? EVisibility::Visible : EVisibility::Collapsed;
				})
